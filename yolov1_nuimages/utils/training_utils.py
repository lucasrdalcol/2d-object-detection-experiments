...
def train_epoch(train_dataloader, model, optimizer, loss_fn, device="cuda"):
    """
    Trains the model for one epoch using the provided training dataloader.
    ...
    Args:
        ...
        loss_fn (callable): The loss function used for computing the loss.
        device (str, optional): The device to be used for training. Defaults to "cuda".

    Returns:
        float: The mean loss over all batches of the training data.
    """
...
def convert_cellboxes(predictions, split_size=7, num_boxes=2, num_classes=25):
    """
